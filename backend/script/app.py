...
@app.route('/')
def serve_vue_app():
    return send_from_directory('../frontend/sudoku/dist', 'index.html')

@app.route('/<path:path>')
def serve_static(path):
    return send_from_directory('../frontend/sudoku/dist', path)
...
def upload_file():
    ...
    if file:
        ...
        try:
            result = subprocess.run(['python', 'crop.py', file_path], 
...
def solve_sudoku():
    ...
    try:
        result = subprocess.run(['python', 'sudoku.py', sudoku_json], 
                                capture_output=True, text=True, check=True)
        solved_sudoku = json.loads(result.stdout)
